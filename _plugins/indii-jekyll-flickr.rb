...
def self.flickr_setup(site)
    ...
    if !site.config['flickr']['size_video']
        site.config['flickr']['size_video'] = '720p'
    end
    ...
    if not site.config['flickr']['use_cache']
        ...
        $flickr = Flickr.new FlickrSecrets::API_KEY, FlickrSecrets::API_SECRET
...
class Photoset
    ...
    def gen_html
        content = '<div class="pswp-gallery">'
...
class Photo
    attr_accessor :id, :title, :slug, :date, :description, :tags, :url_full, :url_thumb, :full_width, :full_height, :cache_file, :position
    ...
    def flickr_load(site, photoset, flickr_photo, pos)
        ...
        if flickr_sizes
            ...
            # replace url with video if it exists
            size_video = flickr_sizes.find {|s| s.label == site.config['flickr']['size_video']}
            if size_video
                size_full = size_video
            end
            if size_full
                self.url_full = size_full.source
                self.full_width = size_full.width
                self.full_height = size_full.height
            end
    ...
    def cache_load(file)
        ...
        self.full_width = cached['full_width']
        self.full_height = cached['full_height']
        self.cache_file = cached['cache_file']
    ...
    def cache_store
        ...
        cached['full_width'] = self.full_width
        cached['full_height'] = self.full_height
        cached['cache_file'] = self.cache_file
    ...
    def gen_thumb_html
        ...
        if self.url_full and self.url_thumb
            # lightbox
            # content = "<a href=\"#{self.url_full}\" data-lightbox=\"photoset\"><img src=\"#{self.url_thumb}\" alt=\"#{self.title}\" title=\"#{self.title}\" class=\"photo thumbnail\" width=\"145\" height=\"145\" /></a>\n"
            # photoswipe
            content =
                "<a href=\"#{self.url_full}\" target=\"_blank\" style=\"text-decoration : none\"
                    data-pswp-width=\"#{self.full_width}\"
                    data-pswp-height=\"#{self.full_height}\"
                    #{self.url_full.include?(".jpg") ? "" : "data-pswp-type=\"video\""}>
                    <img src=\"#{self.url_thumb}\" alt=\"#{self.title}\" width=\"145\" height=\"145\"
                        style=\"object-fit: cover\" class=\"mb-1\" />
                </a>"
        end
        return content
    end
...
class FlickrPageGenerator < Generator
    ...
    def generate(site)
        ...
        file_photosets.each_with_index do |file_photoset, pos|
            ...
            if site.config.dig('flickr', 'generate_photosets')&.include?(photoset.title)
